...
#| label: piezas defectuosas-bernoulli
# piezas defectuosas
p_piezadef <- 0.02

#Inciso a
# Crear tabla de probabilidades
bernoulli_piezadefectuosa <- data.frame(
  Resultado = c("Pieza no defectuosa (0)", "Pieza defectuosa (1)"),
  Valor = c(0, 1),
  Probabilidad = c(1-p_piezadef, p_piezadef)
)

kable(bernoulli_piezadefectuosa, 
      caption = "Distribución Bernoulli - pieza defectuosa (p = 0.02)",
      align = "ccc") %>%
  kable_styling(bootstrap_options = "striped")

#Inciso b
# Parámetros
media_piezadef <- p_piezadef
varianza_piezadef <- p_piezadef * (1 - p_piezadef)
desv_std_piezadef <- sqrt(varianza_piezadef)

cat("Parámetros estadísticos:\n")
cat("Media:", media_piezadef, "\n")
cat("Varianza:", varianza_piezadef, "\n")
cat("Desviación estándar:", desv_std_piezadef, "\n")
...
prob_2_defectuosas <- dbinom(2, size = n_piezas, prob = p_piezadef)

cat("Probabilidad de encontrar exactamente 2 defectuosas\n")
cat("Probabilidad:", prob_2_defectuosas, "\n")
...
# Modelo Bernoulli
p_acertar <- 1/4
cat("Inciso b La probabilidad de acertar:", p_acertar, "\n")
...
bernoulli_acertar <- data.frame(
  ...
  Valor = c(0, 1),
  Probabilidad = c(1-p_acertar, p_acertar)
)
...
#Inciso c
# Parámetros
media_acertar <- p_acertar
varianza_acertar <- p_acertar * (1 - p_acertar)
desv_std_acertar <- sqrt(varianza_acertar)

cat("Parámetros estadísticos:\n")
cat("Media:", media_acertar, "\n")
cat("Varianza:", varianza_acertar, "\n")
cat("Desviación estándar:", desv_std_acertar, "\n")
```
:::
...
```{r}
#| label: lanzamiento_moneda_2
# Parámetros
n_lanzamientos <- 15
p_cara <- 0.5

# Preguntas de interés
# 1. ¿Cuál es la probabilidad de que exactamente 8 caras?
prob_8_caras <- dbinom(8, n_lanzamientos, p_cara)

# 2. ¿Cuál es la probabilidad de que al menos 12 caras?
prob_min_12 <- 1 - pbinom(11, n_lanzamientos, p_cara)

# Parámetros estadísticos
media_cara <- n_lanzamientos * p_cara
varianza_cara <- n_lanzamientos * p_cara * (1 - p_cara)
desv_std_cara <- sqrt(varianza_cara)

cat("P(exactamente 8 caras):", round(prob_8_caras, 4), "\n")
cat("P(al menos 12 caras):", round(prob_min_12, 4), "\n")
cat("Parámetros estadísticos:\n")
cat("Media:", media_cara, "\n")
cat("Varianza:", varianza_cara, "\n")
cat("Desviación estándar:", desv_std_cara, "\n")
...
cat("Número esperado de pacientes curados:", media_curas, "\n")
...
:::(#sol-ejercicio binomial)

```{r}
# Generamos datos  variables independientes binomiales y sumarlas
...
datos_comparacion <- data.frame(
 ...
 datos_final <- rbind(datos_comparacion, datos_compara_2)
 
# Visualización comparativa
ggplot(datos_final, aes(x = valor, fill = Tipo)) +
  geom_histogram(position = "dodge", alpha = 0.6, center = 0, binwidth = 1) +
  scale_fill_manual(values = c("Suma de binomiales" = "#41b7c4", "Binomial directo" = "#86d0b9")) +
