|                                        |
|----------------------------------------|
| itle: "Distribución Uniforme Discreta" |
| ang: es                                |
...
valores <- x_1:x_8
probabilidades <- rep(1/length(valores), length(valores))
prob_par <- sum(probabilidades[valores %% 2 == 0])


cat("Probabilidad número par:", prob_par )
```
:::
...
```{r}
#| code-fold: true
#| label: simulacion ejercicio propuesto
# Función para generar números de distribución uniforme discreta
runif_disc <- function(n, a, b) {
  sample(a:b, n, replace = TRUE)
}
