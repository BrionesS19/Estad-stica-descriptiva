...
**Teoría esperada:** - Para muestras de tamaño $n$ de $X \sim \text{Poisson}(\lambda)$ - $\bar{X} \sim N\left(\lambda, \frac{\lambda}{n}\right)$ cuando $n$ es grande - $E(\bar{X}) = \lambda$ y $\text{Var}(\bar{X}) = \frac{\lambda}{n}$
...
# d) P(X mas de 10)
lambda_mejora <- 1.2
lambda_500_mejora <- lambda_mejora*500
prob_mas_10_mejora <- 1- ppois(10, lambda_500)


cat("a) La probabilidad de recibir exactamente 8 defectos en 500 productos es:", 
    prob_8, "\n")

cat("b) La probabilidad de tener 10 o mas defectos en 500 productos es:",
    prob_mas_10, "\n")

cat("d) La probabilidad de tener 10 o mas defectos en 500 productos si la empresa mejora su proceso a 1.2 defectos por 100 piezas es:",
   prob_mas_10_mejora, "\n")
...
# b) P(X = 0) en 2 hrs
lambda_2_horas <- lambda*2
prob_0 <- dpois(0, lambda_2_horas)

# c) P para superar la capacidad
lambda_4_horas <- lambda*4
prob_sup_capacidad <- 1- ppois(8, lambda_4_horas)

# d)  Capacidad minima
capacidad_minima <- qpois(0.90, lambda_4_horas)
...
# a) P(X >25)
prob_degradacion <- 1- ppois(25, lambda)

# b) P(X >50)
lambda_50 <- lambda*2
prob_50_solicitudes <- 1- ppois(50, lambda_50)

# c) Capacidad óptima
capacidad_optima <- qpois(0.99, lambda)

# d)  Simulación-Porcentaje degradación
set.seed(123)
simulacion <- rpois(60, lambda)
degradacion <- sum(simulacion > capacidad_optima)
porcentaje_degradacion <- (degradacion / 60) * 100


cat("a) La probabilidad de que en un minuto el servidor experimente degradación:", 
    round(prob_degradacion, 4), "\n")
...
cant_unidades <- qpois(0.95, lambda)
