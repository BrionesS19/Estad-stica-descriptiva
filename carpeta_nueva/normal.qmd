...
# 1) Simulación
muestra_temp <- rnorm(1000, mean = mu_proceso, sd = sigma_proceso)

# Estadísticas
media_temp <- mean(muestra_temp)
sd_temp <- sd(muestra_temp)

cat("Media muestral:", media_temp, "\n")
cat("Desviación estándar muestral:", sd_temp, "\n")

# 2) Porcentaje fuera de especificación
fuera_muestra <- mean(muestra_temp < lim_inf | muestra_temp > lim_inf)*100
fuera_teorico <- (pnorm(lim_inf, mu_proceso, sigma_proceso) +
                  (1 - pnorm(lim_sup, mu_proceso, sigma_proceso))) * 100

cat("Porcentaje fuera (simulación):", round(fuera_muestra, 4), "%\n")
cat("Porcentaje fuera (teórico):", round(fuera_teorico, 4), "%\n")

# 3) Descentra a $\mu = 355°C$
mu_descentrado <- 355

descentrado <- (pnorm(lim_inf, mu_descentrado, sigma_proceso) +
                (1 - pnorm(lim_sup, mu_descentrado, sigma_proceso))) * 100

cat("Porcentaje fuera de especificación con μ = 355:", 
    round(descentrado, 4), "%\n")

# 4) sigma  para tener 99.9% de productos dentro de especificación

z <- qnorm(0.999)
sigma_requerido <- (lim_sup - mu_proceso) / z

cat("Sigma requerido para 99.9% dentro:", round(sigma_requerido, 4), "\n")
...
# Estadísticas
media_sim <- mean(df_ventas$ventas)
mediana_sim <- median(df_ventas$ventas)
sd_sim <- sd(df_ventas$ventas)

# 2) P> 60,000
p_mas_60000 <- 1 - pnorm(60000, mean = mu, sd = sigma)

# 3) P < 30,000
p_menos_30000 <- pnorm(30000, mean = mu, sd = sigma)
esperado_meses_menos_30000 <- n_meses * p_menos_30000

# 4) Rango intercuartílico teórico
z25 <- qnorm(0.25)   
z75 <- qnorm(0.75)  
Q1_teo <- mu + z25 * sigma
Q3_teo <- mu + z75 * sigma
IQR_teo <- Q3_teo - Q1_teo

# 5) Meta > 45,000: probabilidad teórica
p_supera_45000 <- 1 - pnorm(45000, mean = mu, sd = sigma)

cat("Media muestral",media_sim , "\n" )
cat("Desviación estandar muestral",sd_sim , "\n" )
cat("Probabilidad de tener ventas superiores a 60,000 pesos",p_mas_60000, "\n" )  
cat("Cuántos meses se espera tener ventas por debajo de 30,000 pesos", esperado_meses_menos_30000,"\n" ) 
cat ("rango intercuartílico de las ventas", IQR_teo,"\n" )
cat("Si la meta es superar 45,000 pesos, ¿qué porcentaje de meses se cumple?", p_supera_45000, "\n" )
```
...
# 1) Simular 800 vidas de batería
n_sim <- 800
vidas <- rnorm(n_sim, mean = mu, sd = sigma)

df_vidas <- tibble(vida = vidas)

# Estadísticos muestrales
media_muestral <- mean(df_vidas$vida)
mediana_muestral <- median(df_vidas$vida)
sd_muestral <- sd(df_vidas$vida)

cat("Media muestral:", round(media_muestral, 3), "horas\n")
cat("Mediana muestral:", round(mediana_muestral, 3), "horas\n")
cat("SD muestral:", round(sd_muestral, 3), "horas\n\n")

# 2) Garantía que cubre 95%
t_95 <- qnorm(0.05, mean = mu, sd = sigma)   # qnorm(0.05) porque queremos 5% por debajo
cat("Garantía (= nivel que cubre 95%):", round(t_95, 3), "horas\n\n")

# 3) Si se venden 10000 con garantía de 900 h: número esperado de fallos
n_vendidas <- 10000
prob_falla_900 <- pnorm(900, mean = mu, sd = sigma)
esperado_fallos <- n_vendidas * prob_falla_900
cat("Esperado fallos (10,000 unidades, garantía 900 h):", round(esperado_fallos), "baterías\n\n")

# 5) Percentil 10 teórico
p10 <- qnorm(0.10, mean = mu, sd = sigma)
cat("Percentil 10 (teórico):", round(p10, 3), "horas\n\n")
...
df_err <- tibble(error = errores)

# 2) % de errores entre -5 y +5
porc_entre_menos5_5_emp <- mean(df_err$error >= -5 & df_err$error <= 5) * 100
porc_entre_menos5_5_teo <- (pnorm(5, mean = mu, sd = sigma) - pnorm(-5, mean = mu, sd = sigma)) * 100

# 3) Probabilidad de |error| > 6
prob_abs_mayor_6_emp <- mean(abs(df_err$error) > 6)
prob_abs_mayor_6_teo <- 2 * (1 - pnorm(6, mean = mu, sd = sigma))

# 4) % de mediciones "precisas" (|error| <= 3)
porc_precisas_emp <- mean(abs(df_err$error) <= 3) * 100
porc_precisas_teo <- (pnorm(3, mean = mu, sd = sigma) - pnorm(-3, mean = mu, sd = sigma)) * 100

# 5) Verificar simetría: calcular media, mediana, skewness y KS entre errores y -errores
media_emp <- mean(df_err$error)
mediana_emp <- median(df_err$error)
skew_emp <- skewness(df_err$error)

cat("Porcentaje entre -5 y +5 (emp):", round(porc_entre_menos5_5_emp, 3), "%\n")
cat("Porcentaje entre -5 y +5 (teo):", round(porc_entre_menos5_5_teo, 3), "%\n\n")
cat("Prob(|error| > 6) (emp):", round(prob_abs_mayor_6_emp, 4), "\n")
cat("Prob(|error| > 6) (teo):", round(prob_abs_mayor_6_teo, 4), "\n\n")
cat("Porcentaje 'precisas' (|error| <= 3) emp:", round(porc_precisas_emp, 3), "%\n")
cat("Porcentaje 'precisas' (teo):", round(porc_precisas_teo, 3), "%\n\n")
...
n_A <- 200
n_B <- 200
cal_A <- rnorm(n_A, mean = mu_A, sd = sd_A)
cal_B <- rnorm(n_B, mean = mu_B, sd = sd_B)

df_cal <- tibble(
  grupo = rep(c("A", "B"), times = c(n_A, n_B)),
  score = c(cal_A, cal_B)
)

# 2) % del Grupo A que supera la media del Grupo B
porc_A_supera_B <- (1 - pnorm(mu_B, mean = mu_A, sd = sd_A)) * 100

# 3) Probabilidad teórica de que un estudiante del Grupo B tenga puntuación < 70
prob_B_menor_70 <- pnorm(70, mean = mu_B, sd = sd_B) * 100
# empírica
0

# 5) Puntuación está el 10% superior de cada grupo
p90_A <- qnorm(0.9, mean = mu_A, sd = sd_A)
p90_B <- qnorm(0.9, mean = mu_B, sd = sd_B)

# Mostrar resultados
cat("1) % Grupo A que supera media de B (mu_B =", mu_B, ")\n")
cat("- Teórico:", round(porc_A_supera_B, 2), "%\n\n")

cat("2) P(B < 70)\n")
cat("- Teórico:", round(prob_B_menor_70, 2), "%\n\n")

cat("3) Percentil 90\n")
cat("- Grupo A: ", round(p90_A, 2), "\n")
cat("- Grupo B: ", round(p90_B, 2), "\n")
