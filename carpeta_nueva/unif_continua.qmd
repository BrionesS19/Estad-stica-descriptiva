...
tiempo_carga <- runif(n_cargas, min = tiempo_1, max = tiempo_2)

# Histograma con densidad teórica
histograma<- ggplot(data.frame(tiempo = tiempo_carga), aes(x = tiempo)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30,
                 fill = "#458B74", color = "black", alpha = 0.7) +
  geom_hline(yintercept = 1/(tiempo_2 - tiempo_1),
             color = "#8B7D6B", size = 1, linetype = "dashed") +
  labs(title = "Histograma de Tiempos de Cargaa",
       x = "Tiempo de carga (segundos)",
       y = "Densidad",
       caption = "Línea roja: Densidad teórica = 1/6") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

# Percentil 75
percentil_75 <- qunif(0.75, min = tiempo_1,max = tiempo_2)
cat("Percentil 75 de los tiempos de carga:", round(percentil_75,
    2), "segundos\n")

# Probabilidad de cargar en menos de 4 segundos
prob_menos_4 <- punif(4, min = tiempo_1, max = tiempo_2)
...
`
```{r}
#| label: ejercicio_calificaciones
...
calificaciones <- runif(n_estudiantes, min = calif_1, max = calif_2)
...
porcentajes_categorias <- table(categorias)
porcentajes_categorias <- prop.table(porcentajes_categorias) * 100

# Porcentajes teóricos
porcentajes_teoricos <- c(
  Aprobado = (80 - 70) / (calif_2 - calif_1) * 100,
  Bien = (90 - 80) / (calif_2 - calif_1) * 100,
  Excelente = (100 - 90) / (calif_2 - calif_1) * 100
)
...
ggplot(df_calificaciones, aes(x = calificaciones, fill = categoria)) +
  ...
  scale_fill_manual(values = c("Aprobado" = "#0000FF", "Bien" = "cadetblue2", "Excelente" = "#008B8B")) +
...
set.seed(123)
n_puntos <- 10000
# Generar puntos aleatorios [-1,1] x [-1, 1]
x <- runif(n_puntos, min = -1, max = 1)
y <- runif(n_puntos, min = -1, max = 1)
# Calcular si caen dentro del círculo unitario
dentro_circulo <- (x^2 + y^2) <= 1

# Estimar π
pi_estimado <- 4 * mean(dentro_circulo)
cat("Estimación de π con simulación Monte Carlo", n_puntos,
    "puntos:", round(pi_estimado, 6), "\n")
# Visualización
...
ggplot(df_pi, aes(x = x, y = y, color = dentro)) +
  ...
  labs(title = "Simulación Monte Carlo para Estimar π",
       subtitle = paste("Estimación π =", round(pi_estimado, 5)),
       x = "X",
       y = "Y",
  ...
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5)) +
  scale_color_manual(values = c("TRUE" = "#528B8B", "FALSE" = "#B22222"))
```
