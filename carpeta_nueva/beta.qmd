...
media_teo <- media_teorica(parametro_a, parametro_b)
varianza_teo <- varianza_teorica(parametro_a, parametro_b)
for (i in seq_along(n_sizes)) {
  n <- n_sizes[i]
  muestra <- rbeta(n,parametro_a, parametro_b)
...
p_media <- ggplot(df_convergencia, aes(x = n, y = media_muestral)) +
  geom_line(color = "#B03060", size = 1) +
  geom_hline(yintercept = media_teo, linetype = "dashed", color = "red") +
  labs(
    title = "Convergencia de la Media Muestral a la Teórica",
...
varianza <- ggplot(df_convergencia, aes(x = n, y = varianza_muestral)) +
  geom_line(color = "#8B5F65", size = 1) +
  ...
  theme(plot.title = element_text(size = 14, face = "bold")) +
  ylim(0, 0.1)
# Mostrargráficas juntas
grid.arrange(p_media, varianza, ncol = 2)
# Histograma para la muestra más grande
n_grande <- max(n_sizes)
muestra_grande <- rbeta(n_grande,parametro_a ,parametro_b)
df_hist <- data.frame(x = muestra_grande)
p_hist <- ggplot(df_hist, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "#FF6EB4", color = "black", alpha = 0.7) +
  stat_function(fun = dbeta, args = list(shape1 = parametro_a , shape2 = parametro_b), color = "#CD5555", size = 1) +
  labs(
...
plots_tcl <- list()
for (n in n_sizes) {
  medias_muestrales <- generar_medias(n, parametro_a,parametro_b, n_replicas)
  df_tcl <- data.frame(medias = medias_muestrales)
  media_teo <- parametro_a / (parametro_a + parametro_b)
  varianza_teo <- (parametro_a * parametro_b) / ((parametro_a + parametro_b)^2 * (parametro_a + parametro_b + 1))
  sd_teo <- sqrt(varianza_teo / n)
  p <- ggplot(df_tcl, aes(x = medias)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "#FFDAB9", color = "palevioletred3", alpha = 0.7) +
    stat_function(fun = dnorm, args = list(mean = media_teo, sd = sd_teo), color = "#FFDAB9", size = 1) +
    labs(
      title = paste("TCL: n =", n),
      subtitle = paste("Media Teórica =", round(media_teo, 4),
