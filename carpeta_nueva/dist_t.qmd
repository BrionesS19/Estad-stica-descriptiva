...
#Grafico muestra 1
ggplot(df_muestra1)+
  geom_histogram(aes(x=X, y = after_stat(density)), bins = 50, fill="#3CB371", color="black", alpha = 0.7)+
  stat_function(fun = function(x) dt(x, df = n), color="#191970", linewidth = 1.2)+
  stat_function(fun = function(x) dnorm(x), color= "#EE82EE", linewidth = 1.2)+
  theme_bw()
...
```{r}
media_1 <- function(muestra1){
  muestra <- rnorm(muestra1)
  media <- mean(muestra)
  sd <- sd(muestra)
  return(c(media, sd))
}

valores_muestra1 <- replicate(100, media_1(muestra1))

media_2 <- function(muestra2){
  muestra2 <- rnorm(muestra2)
  media <- mean(muestra2)
  sd <- sd(muestra2)
  return(c(media, sd))
}

valores_muestra2 <- replicate(300, media_2(muestra2))

media_3 <- function(muestra3){
  muestra3 <- rnorm(muestra3)
  media <- mean(muestra3)
  sd <- sd(muestra3)
  return(c(media, sd))
}

valores_muestra3 <- replicate(1000, media_3(muestra3))
...
df_ejercicio <- tibble(
  ...
  X = rnorm(num_muestras),
  Ji = rchisq(num_muestras, df = n), 
  T = X/sqrt(Ji/n)
)

#Histograma de T muestra con densidad teorica t(n)
ggplot(df_ejercicio)+
  geom_histogram(aes(x=T, y = after_stat(density)), bins = 50, fill="#778899", color="black", alpha = 0.7)+
  stat_function(fun = function(x) dt(x, df = n), color="#F08080", linewidth = 1.2)+
  stat_function(fun = function(x) dnorm(x), color= "#E9967A", linewidth = 1.2)+
  theme_bw()
```
...
ggplot(df_T, aes(x = T)) +
  geom_histogram(aes(y = ..density..), 
                 bins = 40, fill = "#8A2BE2", color = "black", alpha = 0.7) +
  stat_function(fun = function(x) dt(x, df = gl), 
                color = "#00008B", linewidth = 1.2) +
  labs(
    title = "Histograma de T con la densidad teórica t(10)",
    x = "T",
    y = "Densidad"
  ) +
  theme_bw()
...
valores <- replicate(N, media_sd(n, mu, sigma))
...
df_exr2 <- tibble(
  ...
  Media = valores[1, ],
  SD = valores[2, ],
  T = ((Media - mu)/(SD/sqrt(n)))
  )
  
ggplot(df_exr2)+
  geom_histogram(aes(x=T, y = after_stat(density)), bins = 100, fill="#008B8B", color = "black", alpha = 0.7)+
  geom_function(fun = function(x) dt(x, df=n_mue-1), color ="#2F4F4F", linewidth=1.2)+
  theme_bw()
```
```
