...
media_teo <- alpha / lambda
var_teo   <- alpha / (lambda^2)

# Tamaños de muestra
n_muestras <- c(5, 10, 20, 30, 50, 100)
s <- 1000 

lista_gamma <- lapply(n_muestras, function(n) {
  
  # Simular  medias muestrales
  medias <- replicate(s, mean(rgamma(n, shape = alpha, rate = lambda)))
  
  tibble(
    n = n,
    medias = medias,
    media_teorica = media_teo,
    var_teorica_media = var_teo / n,
...
df_gamma_tcl <- bind_rows(lista_gamma)

df_gamma_tcl$n_factor <- factor(paste("n =", df_gamma_tcl$n))

# Histogramas + Normal teórica TCL

ggplot(df_gamma_tcl, aes(x = medias)) +
  geom_histogram(aes(y = after_stat(density)), 
                 bins = 30, fill = "#9AC0CD", color = "black", alpha = 0.7) +
  stat_function(
    fun = function(x) dnorm(x, mean = media_teo, sd = sqrt(var_teo / unique(df_gamma_tcl$n))),
    color = "#8B4789",
    linewidth = 1
  ) +
  facet_wrap(~n_factor, scales = "free", ncol = 3) +
  labs(
    title = "Teorema Central del Límite — Medias Muestrales Gamma(2.5, 1.8)",
    subtitle = "Comparación entre media muestral y Normal teórica del TCL",
    x = "Media muestral",
    y = "Densidad"
  ) +
  theme_minimal()
...
# Resumens
resumen_gamma <- df_gamma_tcl %>%
  group_by(n) %>%
  summarise(
    media_muestral = mean(medias),
    media_teorica = first(media_teorica),
    var_media_muestral = var(medias),
    var_media_teorica = first(var_teorica_media),
...
# Simulación sumas de 5 exponenciales Exp(1.5)
suma_exp <- replicate(s, {
  sum(rexp(n, rate = lambda))
})
...
# Histograma + densidad teórica Gamma(n, lambda)
ggplot(df_suma, aes(x = suma)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 40,
                 fill = "#9AC0CD",
                 color = "black",
                 alpha = 0.7) +
  stat_function(
    fun = function(x) dgamma(x, shape = n, rate = lambda),
    color = "#8B4789",
    linewidth = 1.2
  ) +
  labs(
    title = "Suma de Exponenciales vs. Densidad Gamma Teórica",
    subtitle = paste("Suma de", s, "muestras de", n, "Exp(", lambda, ")"),
    x = "Suma",
    y = "Densidad"
  ) +
  theme_minimal()

# Simulación sumas de 10 exponenciales Exp(1.5)
suma_exp_1 <- replicate(s, {
  sum(rexp(n_1, rate = lambda))
})
...
# Histograma + densidad teórica Gamma(n_1, lambda)
ggplot(df_suma_1, aes(x = suma_1)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 40,
                 fill = "#9AC0CD",
                 color = "black",
                 alpha = 0.7) +
  stat_function(
    fun = function(x) dgamma(x, shape = n_1, rate = lambda),
    color = "#8B4789",
    linewidth = 1.2
  ) +
  labs(
    title = "Suma de Exponenciales vs. Densidad Gamma Teórica",
    subtitle = paste("Suma de", s, "muestras de", n_1, "Exp(", lambda, ")"),
    x = "Suma",
    y = "Densidad"
  ) +
  theme_minimal()

# Simulación sumas de 20 exponenciales Exp(1.5)
suma_exp_2 <- replicate(s, {
  sum(rexp(n_2, rate = lambda))
})
...
ggplot(df_suma_2, aes(x = suma_2)) +
  geom_histogram(aes(y = after_stat(density)),
                 ...
                 alpha = 0.7) +
  stat_function(
    fun = function(x) dgamma(x, shape = n_2, rate = lambda),
    color = "#8B4789",
    linewidth = 1.2
  ) +
  labs(
    title = "Suma de Exponenciales vs. Densidad Gamma Teórica",
    subtitle = paste("Suma de", s, "muestras de", n_2, "Exp(", lambda, ")"),
    x = "Suma",
    y = "Densidad"
  ) +
  theme_minimal()
